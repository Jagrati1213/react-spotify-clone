...
function Index() {
  ...
  useEffect(()=>{
    apiClient.get('me/playlists').then(res => setPlayList(res.data.items)).catch((err)=>{ console.log(err)})
   },[playList]);
     ...
     (
        <>
         <div className="library_body w-full h-full grid gap-7 flex-wrap justify-between">
            {
              playList.map((item)=>{
                  return <div className='playlist_card rounded-md text-t_white flex flex-col text-center my-3' key={item.id}
